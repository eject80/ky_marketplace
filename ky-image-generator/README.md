...
(주)아이비김영 재직자용 이미지 생성 MCP 도구. 텍스트 프롬프트(+참조 이미지)를 받아 Gemini로 이미지를 생성하고, 결과를 Google Drive에 업로드해 파일 ID와 미리보기 링크를 반환한다. 과거에 생성한 이미지 이력도 날짜·키워드로 조회할 수 있다.

이 플러그인은 스킬 없이 MCP 서버 연결만 제공한다 — 설치하면 `ky-image-generator` MCP 서버가 등록되고, `generate_image`·`generated_images` 툴을 대화 중에 바로 호출할 수 있다.
...
### 생성 이력 조회 (`generated_images`)

"내가 최근에 생성한 이미지 보여줘" 같은 요청에 호출된다. 입력 가능한 옵션:

- `target_date` — 조회할 날짜 (한국시간 `YYYY-MM-DD`, 생략 시 전체 날짜)
- `query` — 프롬프트에서 검색할 키워드 (생략 시 검색 안 함)
- `limit` — 페이지당 최대 로그 수, 기본 `10` (`1`~`100`)
- `page` — 조회할 페이지 번호, 기본 `1`

결과는 각 로그 항목(`timestamp`, `input_prompt`, `output_result`) 목록과 `page`, `limit`, `has_next`, `has_previous`를 포함한 형태로 반환된다.
